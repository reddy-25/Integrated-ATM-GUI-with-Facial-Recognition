...
class BankUi:
    def __init__(self,root):
        self.root = root
        self.account_sid = 'AC68d642ae47b9b23c7f326e7114d8c417'
        self.auth_token = 'Show'
        self.phone_number = "+18135445851"
    ...
    def begin_page(self):
        ...
        self.withdraw = Button(self.frame, text="Withdraw Money",bg="#50A8B0",fg="white",font=ARIAL,command=self.enter_unique_id)
        self.q = Button(self.frame, text="Quit", bg="#50A8B0", fg="white", font=ARIAL, command=self.root.destroy)
    ...
    def enter_unique_id(self):
        self.frame.destroy()
        self.frame = Frame(self.root, bg="#0019fc", width = 900, height = 500)
        root.geometry("800x500")
        self.user_name = Label(self.frame, text="Unique Id",bg="#0019fc",fg="white",font=ARIAL)
        self.user_entry =  Entry(self.frame,bg="honeydew",highlightcolor="#50A8B0",
           highlightthickness=2,
            highlightbackground="white")
        self.button = Button(self.frame,text="Go to verify Face Id",bg="#50A8B0",fg="white",font=ARIAL,command=self.verify_user_name)
        self.q = Button(self.frame,text="Quit",bg="#50A8B0",fg="white",font=ARIAL,command = self.root.destroy)
        self.b = Button(self.frame,text="Back",bg="#50A8B0",fg="white",font=ARIAL,command = self.begin_page)
        self.user_name.place(x=125,y=100,width=120,height=20)
        self.user_entry.place(x=153,y=130,width=200,height=20)
    ...
    def verify_user_name(self):
        ...
        pres_name = data.loc[:,'unique_id']==int(self.uname)
        temp = 0
        for i in pres_name:
            if(i==True):
                temp += 1
                break
        if(temp==0):
            messagebox._show("Error","There is no such unique id registered in this bank")
            self.enter_unique_id()
        else:
            print("Going to verification page")
            self.mobile_no = data[data.loc[:,'unique_id']==int(self.uname)].loc[:,'mobile_number'].values[0]
            print(self.mobile_no)
            self.withdraw_money_page()

    def withdraw_money_page(self):
        print("entered verification page")
        self.frame.destroy()
        self.frame = Frame(self.root,bg="#0019fc",width=900,height=500)
    ...
    def enroll_user(self):
        ...
        self.pentry = Entry(self.frame,bg="honeydew",show="*",highlightcolor="#50A8B0",
           highlightthickness=2,
            highlightbackground="white")
        # inserted lines
        self.phlabel = Label(self.frame, text="Mobile number" ,bg="#0019fc",fg="white",font=ARIAL)
        self.phentry = Entry(self.frame, bg="honeydew",highlightcolor="#50A8B0",
           highlightthickness=2,
            highlightbackground="white")
        # end inserted lines
        ...
        # inserted lines
        self.phlabel.place(x=425, y=100,width=120,height=20)
        self.phentry.place(x=423,y=130,width=200,height=20)
        # end inserted lines
    ...
    def enroll_and_move_to_next_screen(self):
        ...
        mobile_number = self.phentry.get()
        if not name and not password and not mobile_number:
            messagebox._show("Error", "You need a name to enroll an account and you need to input  password and mobile number!")
            self.enroll_user()
        elif not password:
            ...
            messagebox._show("Error", "You need to enter a mobile number")
            self.enroll_user()
        #elif str(mobile_number)[0] != '+':
         #   messagebox._show("Error", "You need to enter a mobile number with country code.For India it is +91")
          #  self.enroll_user()
        #elif (str(mobile_number))[0:3] == '+91':
        if len(str(mobile_number)) != 10:
            messagebox._show("Error", "You need to enter 10 digit mobile number")
            self.enroll_user()
        else:
            self.write_to_csv()
    ...
    def user_account_transfer_transc(self):
        ...
        if int(self.entry11.get()) not in data['account_number'].values:
            ...
            # inserted
            sender = update_data.loc[data[data.loc[:,'unique_id'] == self.real_user].loc[:,'account_number'].values[0],'mobile_number']
            sent = self.entry21.get()
            total_sender_money = update_data.loc[data[data.loc[:,'unique_id'] == self.real_user].loc[:,'account_number'].values[0],'account_balance']
            total_receiver_money = update_data.loc[int(self.entry11.get()),'account_balance']
            receiver = update_data.loc[int(self.entry11.get()),'mobile_number']
            #end inserted
            update_data['account_number'] = update_data.index
            update_data.reset_index(drop = True, inplace= True)
            update_data = update_data.reindex(labels = ['unique_id','account_number','name','bank', 'password','account_balance','mobile_number'], axis = 1)
            update_data.to_csv('bank_details.csv',index = None)
            # now sending the message inserted
            import requests
            import json
            ...
            my_data = {
                ...
                'message': str(sent)+" rupees is debited from your account. Your total balance is "+str(total_sender_money),
            ...
            # create a dictionary
            headers = {
                'authorization': '6YBqzcGWPIQEZ8gpR7y1H52edjrhtLCXUlounkOa4iFMbNsJDfkoDJZQSnHI1fMexi7EK6Bh9vUYyjXm',
            ...
            my_data = {
                ...
                'sender_id': 'FSTSMS',
                 ...
                 # Put your message here!
                'message': str(sent)+" rupees is credited to your account. Your total balance is "+str(total_receiver_money),
    ...
    def user_deposit_trans(self,flag):
        ...
        #inserted
        total_balance = update_data.loc[self.real_user,'account_balance']
        receiver = update_data.loc[self.real_user,'mobile_number']
        deposited = self.money_box.get()
        #end
        update_data.reset_index(inplace=True)
        update_data.columns = ['unique_id','account_number','name','bank', 'password','account_balance','mobile_number']
        update_data.to_csv('bank_details.csv',index = None)
        #send msg
        import requests
        import json
        ...
        my_data = {
            ...
            'sender_id': 'FSTSMS',
             ...
             # Put your message here!
            'message': str(deposited)+" rupees is deposited to your account. Your total balance is "+str(total_balance),
        ...
        if int(self.money_box.get()) <= update_data.loc[self.real_user,'account_balance']:
            ...
            my_data = {
                ...
                'sender_id': 'FSTSMS',
                 ...
                 # Put your message here!
                'message': str(withdraw)+" rupees is withdrawn from your account. Your total balance is "+str(total_balance),
    ...
    def write_to_csv(self):
        ...
        if not os.path.exists("bank_details.csv"):
            with open(r'bank_details.csv','a', newline = '\n') as f:
                writer = csv.writer(f)
                writer.writerow(['unique_id','account_number','name','bank', 'password', 'account_balance', 'mobile_number'])
        with open(r'bank_details.csv','a', newline = '\n') as f:
            writer = csv.writer(f)
            writer.writerow([unique_id,account_number,name,bank, password, account_balance, mobile_number])
    ...
    def captureuser(self):
        ...
        self.train_model()
        # inserted
        mobile_no = data.loc[:,'mobile_number'].values[-1]
        import requests
        import json
        ...
        my_data = {
            ...
            'sender_id': 'FSTSMS',
             ...
             # Put your message here!
            'message': 'Face ID successfully registered for your account',
        ...
        while True:
            ...
            for i in range(0, detections.shape[2]):
                ...
                if confidence > 0.5:
                    ...
                    proba = preds[j]
                    name = le.classes_[j]
                    temp = self.uname  # data[data.loc[:,'unique_id']==int(self.uname)].loc[:,'name'].values[0]
                    temp1 = name
                    print(name)
                    print(self.uname)
                    if (str(temp) != str(temp1)) :
                        print('enter')
                        name = 'unknown'
            ...
            if self.countter != 0:
                ...
                my_data = {
                    ...
                    'sender_id': 'FSTSMS',
                     ...
                     # Put your message here!
                    'message': 'Somebody is trying to authenticate your account',
